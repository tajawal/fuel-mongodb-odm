 ...
 * <code>
 * class Model_Post extends Mongo_Odm\Document {
 *   protected $name = 'posts';
 *   // All model-related code here
 * }
 * $post = Mongo_Odm\Document::factory('post', $post_id);
 * </code>
 *
 * 2. Row Data Gateway pattern:
 * <code>
 * class Model_Post_Collection extends Mongo_Odm\Collection {
 *   protected $name = 'posts';
 ...
 *
 * <code>
 *   class Model_Document extends Mongo_Odm\Document {
 *     public $name = 'test';
 *   }
 *   $document = new Model_Document(); // or Mongo_Odm\Document::factory('document');
 *   $document->name = 'Mongo';
 *   $document->type = 'db';
 *   $document->save();
 ...
 *   }
 *
 *   class Model_User extends Mongo_Odm\Document {
 *     protected $name = 'users';
 *   }
 *
 *   $user = Mongo_Odm\Document::factory('user')->set('id','colin')->set('email','colin@mollenhour.com');
 *   $post = Mongo_Odm\Document::factory('post');
 *   $post->user = $user;
 *   $post->title = 'MongoDb';
 *   $post->save()
...
abstract class Document {
	...
	/**
	 * Instantiate an object conforming to Mongo_Odm\Document conventions.
	 ...
	 *
	 *  If using a corresponding Mongo_Odm\Collection subclass, set this only in the Mongo_Odm\Collection subclass.
	 *
	 *  @var  string */
	protected $name;

	/** The database configuration name (passed to Mongo_Odm\Database::instance() )
	 *
	 *  If using a corresponding Mongo_Odm\Collection subclass, set this only in the Mongo_Odm\Collection subclass.
	 *
	 *  @var  string */
	...
	/** A cache of Mongo_Odm\Collection instances for performance
	 *  @static  array */
  ...
  public function __set($name, $value)
  {
    ...
    if(array_key_exists($name, $this->_references))
    {
      ...
      {
        throw new \MongoException('Cannot set reference to object that is not a Mongo_Odm\Document');
      }
   ...
   *
   * @param   string  $action  The type of save action, one of Mongo_Odm\Document::SAVE_*
   */
